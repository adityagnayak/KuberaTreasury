...
from app.services.auth_service import AuthService, ChangePasswordRequest, LoginRequest, TokenResponse
...
async def mfa_verify(payload: MfaVerifyRequest, db: DBSession, user: AuthUser) -> dict:
    ok = await svc.verify_mfa_setup(db, user.tenant_id, user.user_id, payload.code, user.roles)
    if not ok:
...
async def logout_all(db: DBSession, user: AuthUser) -> dict:
    revoked = await svc.revoke_all_sessions(db, user.tenant_id, user.user_id, user.user_id)
    return {"revoked_sessions": revoked}


@router.post("/change-password")
async def change_password(payload: ChangePasswordRequest, db: DBSession, user: AuthUser) -> dict:
    try:
