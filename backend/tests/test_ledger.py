...
def _balanced_journal(period_id, account_id_dr, account_id_cr, amount=Decimal("100")) -> JournalCreate:
...
async def test_balanced_journal_created(
    ...
    base_account, counter_account, open_period,
):
    svc = _svc(db, tenant_id, user_id)
    jnl = await svc.create_journal(
        _balanced_journal(open_period.period_id, base_account.account_id, counter_account.account_id)
    )
...
async def test_journal_blocked_on_soft_closed_period(
    ...
    with pytest.raises(PeriodClosedError):
        await svc.create_journal(
            _balanced_journal(period.period_id, base_account.account_id, counter_account.account_id)
        )
...
async def test_reversal_journal_created(
    ...
    await svc.post_journal(jnl.journal_id)
    reversal = await svc.reverse_journal(jnl.journal_id, open_period.period_id, "REV-001")
...
async def test_reversal_swaps_debit_credit(
    ...
    amount = Decimal("250")
    jnl = await svc.create_journal(
        _balanced_journal(open_period.period_id, base_account.account_id, counter_account.account_id, amount)
    )
    await svc.post_journal(jnl.journal_id)
    reversal = await svc.reverse_journal(jnl.journal_id, open_period.period_id, "REV-002")
    ...
    rev_line = next(l for l in reversal.lines if l.account_id == base_account.account_id)
...
async def test_t0_vat_expansion_generates_vat_lines(
    ...
    from app.services.chart_of_accounts_service import ChartOfAccountsService, AccountCreate
    coa_svc = ChartOfAccountsService(db, tenant_id, user_id)
    # Create required VAT accounts
    input_vat = await coa_svc.create(
        AccountCreate(account_code="1101", account_name="VAT Input", account_type="asset", currency_code="GBP")
    )
    output_vat = await coa_svc.create(
        AccountCreate(account_code="2001", account_name="VAT Output", account_type="liability", currency_code="GBP")
    )
    sales = await coa_svc.create(
        AccountCreate(account_code="4001", account_name="Sales", account_type="income", currency_code="GBP")
    )
    bank = await coa_svc.create(
        AccountCreate(account_code="1001", account_name="Bank", account_type="asset", currency_code="GBP")
    )
