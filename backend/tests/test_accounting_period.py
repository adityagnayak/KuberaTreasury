...
@pytest.mark.asyncio
async def test_get_nonexistent_raises(db: AsyncSession, tenant: Tenant, tenant_id, user_id):
    svc = _svc(db, tenant_id, user_id)
...
@pytest.mark.asyncio
async def test_soft_close_requires_treasury_manager(db: AsyncSession, tenant: Tenant, tenant_id, user_id):
    svc = _svc(db, tenant_id, user_id, roles=[])  # no roles
    p = await svc.create_period(_period_payload("Feb 2026", date(2026, 2, 1), date(2026, 2, 28)))
    with pytest.raises(PermissionDeniedError):
        await svc.soft_close(p.period_id)


@pytest.mark.asyncio
async def test_soft_close_succeeds_with_treasury_manager(db: AsyncSession, tenant: Tenant, tenant_id, user_id):
    svc = _svc(db, tenant_id, user_id, roles=["treasury_manager"])
    p = await svc.create_period(_period_payload("Mar 2026", date(2026, 3, 1), date(2026, 3, 31)))
    closed = await svc.soft_close(p.period_id)
    assert closed.status == "soft_closed"


@pytest.mark.asyncio
async def test_hard_close_requires_system_admin(db: AsyncSession, tenant: Tenant, tenant_id, user_id):
    svc = _svc(db, tenant_id, user_id, roles=["treasury_manager"])  # not system_admin
    p = await svc.create_period(_period_payload("Apr 2026", date(2026, 4, 1), date(2026, 4, 30)))
    with pytest.raises(PermissionDeniedError):
        await svc.hard_close(p.period_id, HardCloseRequest(close_reason="Period closed for audit."))


@pytest.mark.asyncio
async def test_hard_close_succeeds_with_system_admin(db: AsyncSession, tenant: Tenant, tenant_id, user_id):
    svc = _svc(db, tenant_id, user_id, roles=["system_admin"])
    p = await svc.create_period(_period_payload("May 2026", date(2026, 5, 1), date(2026, 5, 31)))
    closed = await svc.hard_close(p.period_id, HardCloseRequest(close_reason="Audit complete."))
    assert closed.status == "hard_closed"
    assert closed.hard_closed_by_user_id == user_id


@pytest.mark.asyncio
async def test_reopen_requires_system_admin(db: AsyncSession, tenant: Tenant, tenant_id, user_id):
    admin_svc = _svc(db, tenant_id, user_id, roles=["system_admin"])
    p = await admin_svc.create_period(_period_payload("Jun 2026", date(2026, 6, 1), date(2026, 6, 30)))
    ...
    with pytest.raises(PermissionDeniedError):
        await user_svc.reopen_period(p.period_id, ReopenRequest(reopen_reason="Needs correction for audit."))


@pytest.mark.asyncio
async def test_reopen_succeeds_with_system_admin(db: AsyncSession, tenant: Tenant, tenant_id, user_id):
    svc = _svc(db, tenant_id, user_id, roles=["system_admin"])
    p = await svc.create_period(_period_payload("Jul 2026", date(2026, 7, 1), date(2026, 7, 31)))
...
@pytest.mark.asyncio
async def test_soft_close_already_closed_raises(db: AsyncSession, tenant: Tenant, tenant_id, user_id):
    svc = _svc(db, tenant_id, user_id, roles=["treasury_manager", "system_admin"])
    p = await svc.create_period(_period_payload("Aug 2026", date(2026, 8, 1), date(2026, 8, 31)))
    await svc.soft_close(p.period_id)
...
async def test_ct_dates_via_service_static_method():
    result = AccountingPeriodService.compute_ct_dates(date(2026, 3, 31), is_large_company=False)
    assert result.ct600_due_date == date(2027, 1, 1)


@pytest.mark.asyncio
async def test_ct_dates_large_company_returns_four_qips():
    result = AccountingPeriodService.compute_ct_dates(date(2026, 12, 31), is_large_company=True)
...
@pytest.mark.asyncio
async def test_period_stores_ct600_due_date(db: AsyncSession, tenant: Tenant, tenant_id, user_id):
    svc = _svc(db, tenant_id, user_id)
    p = await svc.create_period(_period_payload("FY-2026", date(2026, 1, 1), date(2026, 12, 31)))
...
@pytest.mark.asyncio
async def test_list_periods_filter_by_status(db: AsyncSession, tenant: Tenant, tenant_id, user_id):
    svc = _svc(db, tenant_id, user_id, roles=["treasury_manager"])
    p1 = await svc.create_period(_period_payload("P1", date(2026, 1, 1), date(2026, 1, 31)))
    p2 = await svc.create_period(_period_payload("P2", date(2026, 2, 1), date(2026, 2, 28)))
