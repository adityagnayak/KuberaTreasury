...
def upgrade() -> None:
    ...
    hmrc_obligation_status_enum = sa.Enum("open", "fulfilled", "overdue", name="hmrc_obligation_status")
    ...
    op.create_table(
        ...
        sa.Column("classification_level", classification_enum, nullable=False, server_default="OFFICIAL"),
        ...
        sa.CheckConstraint("char_length(accounts_office_reference) = 13", name="ck_tenants_aor_len"),
        sa.CheckConstraint("classification_level <> 'OFFICIAL-SENSITIVE'", name="ck_tenants_no_official_sensitive"),
    ...
    op.create_table(
        ...
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("personal_data_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
    ...
    op.create_table(
        "permissions",
        sa.Column("permission_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="CASCADE"), nullable=False),
        sa.Column("permission_key", sa.String(length=120), nullable=False),
        sa.Column("description", sa.String(length=255), nullable=True),
        sa.UniqueConstraint("tenant_id", "permission_key", name="uq_permissions_tenant_key"),
    )

    op.create_table(
        "user_roles",
        sa.Column("user_role_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="CASCADE"), nullable=False),
        sa.Column("user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.user_id", ondelete="CASCADE"), nullable=False),
        sa.Column("role_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("roles.role_id", ondelete="CASCADE"), nullable=False),
        sa.UniqueConstraint("tenant_id", "user_id", "role_id", name="uq_user_roles_unique"),
    )

    op.create_table(
        "role_permissions",
        sa.Column("role_permission_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="CASCADE"), nullable=False),
        sa.Column("role_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("roles.role_id", ondelete="CASCADE"), nullable=False),
        sa.Column("permission_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("permissions.permission_id", ondelete="CASCADE"), nullable=False),
        sa.Column("effect", permission_effect_enum, nullable=False),
        sa.UniqueConstraint("tenant_id", "role_id", "permission_id", name="uq_role_permission_unique"),
    )

    op.create_table(
        "auth_factors",
        sa.Column("auth_factor_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="CASCADE"), nullable=False),
        sa.Column("user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.user_id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("ip_allowlist_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("tenant_id", "jwt_id", name="uq_auth_sessions_tenant_jwt_id"),
    )

    op.create_table(
        "counterparties",
        sa.Column("counterparty_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.UniqueConstraint("tenant_id", "counterparty_code", name="uq_counterparty_code"),
    )

    op.create_table(
        "bank_accounts",
        sa.Column("bank_account_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("account_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("chart_of_accounts.account_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("counterparty_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("counterparties.counterparty_id", ondelete="SET NULL"), nullable=True),
        ...
        sa.UniqueConstraint("tenant_id", "event_sequence", name="uq_ledger_events_sequence"),
    )
    op.create_index("ix_ledger_events_tenant_effective", "ledger_events", ["tenant_id", "effective_at"])
    op.create_index("ix_ledger_events_tenant_vat_code", "ledger_events", ["tenant_id", "vat_code"])

    op.create_table(
        "ledger_positions",
        sa.Column("position_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("account_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("chart_of_accounts.account_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("as_of", sa.DateTime(timezone=True), nullable=False),
        sa.Column("balance", sa.Numeric(20, 4), nullable=False),
        sa.Column("currency_code", sa.String(length=3), nullable=False),
        sa.UniqueConstraint("tenant_id", "account_id", "as_of", name="uq_ledger_position_snapshot"),
    )

    op.create_table(
        "exchange_rates_hmrc",
        sa.Column("exchange_rate_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("revaluation_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("period_end", sa.Date(), nullable=False),
        sa.Column("account_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("chart_of_accounts.account_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("from_currency", sa.String(length=3), nullable=False),
        sa.Column("to_currency", sa.String(length=3), nullable=False, server_default="GBP"),
        sa.Column("hmrc_exchange_rate_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("exchange_rates_hmrc.exchange_rate_id", ondelete="RESTRICT"), nullable=False),
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.UniqueConstraint("tenant_id", "period_end", "account_id", name="uq_currency_reval_period_account"),
    )

    op.create_table(
        "payment_policies",
        sa.Column("payment_policy_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="CASCADE"), nullable=False),
        ...
        sa.Column("is_active", sa.Boolean(), nullable=False, server_default=sa.text("true")),
        sa.UniqueConstraint("tenant_id", "policy_name", "currency_code", name="uq_payment_policy"),
        sa.CheckConstraint("required_approvals >= 2", name="ck_payment_policy_four_eyes_min"),
    )

    op.create_table(
        "payment_batches",
        sa.Column("payment_batch_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("batch_reference", sa.String(length=60), nullable=False),
        sa.Column("execution_date", sa.Date(), nullable=False),
        sa.Column("channel", payment_channel_enum, nullable=False, server_default="manual_pain001"),
        sa.Column("status", payment_status_enum, nullable=False, server_default="draft"),
        sa.Column("created_by_user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.UniqueConstraint("tenant_id", "batch_reference", name="uq_payment_batch_reference"),
    )

    op.create_table(
        "payment_instructions",
        sa.Column("payment_instruction_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("payment_batch_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("payment_batches.payment_batch_id", ondelete="CASCADE"), nullable=False),
        sa.Column("counterparty_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("counterparties.counterparty_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("debit_bank_account_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("bank_accounts.bank_account_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("amount", sa.Numeric(20, 4), nullable=False),
        sa.Column("currency_code", sa.String(length=3), nullable=False),
    ...
    op.create_table(
        "payment_approvals",
        sa.Column("payment_approval_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("payment_instruction_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("payment_instructions.payment_instruction_id", ondelete="CASCADE"), nullable=False),
        sa.Column("approver_user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.user_id", ondelete="RESTRICT"), nullable=False),
        ...
        sa.UniqueConstraint("tenant_id", "payment_instruction_id", "approver_user_id", name="uq_payment_approver_once"),
    )

    op.create_table(
        "pain001_exports",
        sa.Column("pain001_export_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("payment_batch_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("payment_batches.payment_batch_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("file_name", sa.String(length=255), nullable=False),
        sa.Column("sha256_checksum", sa.String(length=64), nullable=False),
        sa.Column("exported_by_user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_table(
        "tax_profiles",
        sa.Column("tax_profile_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("is_large_company_for_ct", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("ct_due_rule", sa.String(length=40), nullable=False, server_default="9_months_post_year_end"),
        sa.Column("cir_threshold_amount", sa.Numeric(20, 2), nullable=False, server_default="2000000.00"),
    ...
    op.create_table(
        "hmrc_obligations",
        sa.Column("hmrc_obligation_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        ...
        sa.Column("status", hmrc_obligation_status_enum, nullable=False, server_default="open"),
        sa.Column("source", sa.String(length=20), nullable=False, server_default="hmrc_api_v1"),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.UniqueConstraint("tenant_id", "tax_type", "period_key", name="uq_hmrc_obligation_period"),
    )

    op.create_table(
        "hmrc_api_submissions",
        sa.Column("hmrc_submission_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("tax_type", hmrc_tax_type_enum, nullable=False),
        sa.Column("api_version", sa.String(length=10), nullable=False, server_default="v1.0"),
        ...
        sa.Column("submitted_by_user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True),
        sa.Column("submitted_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.Index("ix_hmrc_submission_tenant_tax_time", "tenant_id", "tax_type", "submitted_at"),
    )

    op.create_table(
        "tax_payment_references",
        sa.Column("tax_payment_reference_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("tax_type", hmrc_tax_type_enum, nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("paye_calendar_entry_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        ...
        sa.UniqueConstraint("tenant_id", "tax_month", "tax_year", name="uq_paye_calendar_month_year"),
    )

    op.create_table(
        "corporate_interest_restrictions",
        sa.Column("cir_record_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("period_start", sa.Date(), nullable=False),
        sa.Column("period_end", sa.Date(), nullable=False),
        sa.Column("net_group_interest", sa.Numeric(20, 2), nullable=False),
        sa.Column("threshold", sa.Numeric(20, 2), nullable=False, server_default="2000000.00"),
        sa.Column("is_above_threshold", sa.Boolean(), sa.Computed("net_group_interest > threshold"), nullable=False),
        sa.Column("review_required", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.UniqueConstraint("tenant_id", "period_start", "period_end", name="uq_cir_period"),
    )

    op.create_table(
        "intercompany_transactions",
        sa.Column("intercompany_transaction_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("source_ledger_event_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("ledger_events.event_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("counterparty_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("counterparties.counterparty_id", ondelete="RESTRICT"), nullable=False),
        ...
        sa.CheckConstraint("tp_document_hash IS NULL OR char_length(tp_document_hash)=64", name="ck_tp_doc_hash_len"),
    )

    op.create_table(
        "forecast_cashflows",
        sa.Column("forecast_cashflow_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
    ...
    op.create_table(
        "contracts",
        sa.Column("contract_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("contract_reference", sa.String(length=60), nullable=False),
        sa.Column("customer_name", sa.String(length=255), nullable=True),
        sa.Column("is_mod_contract", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("classification_level", classification_enum, nullable=False, server_default="OFFICIAL"),
        sa.Column("start_date", sa.Date(), nullable=True),
        sa.Column("end_date", sa.Date(), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.CheckConstraint("classification_level <> 'OFFICIAL-SENSITIVE'", name="ck_contracts_no_official_sensitive"),
        sa.UniqueConstraint("tenant_id", "contract_reference", name="uq_contract_reference"),
    )

    op.create_table(
        "record_retention_policies",
        sa.Column("retention_policy_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="CASCADE"), nullable=False),
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.CheckConstraint("retention_years IN (7, 10)", name="ck_retention_years_allowed"),
        sa.UniqueConstraint("tenant_id", "record_type", name="uq_retention_record_type"),
    )
    ...
    op.create_table(
        "audit_export_jobs",
        sa.Column("audit_export_job_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("requested_by_user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_table(
        "ai_inference_logs",
        sa.Column("ai_inference_log_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("request_ref", sa.String(length=120), nullable=False),
        sa.Column("model_name", sa.String(length=80), nullable=False, server_default="claude-sonnet-4-6"),
        sa.Column("input_reference_payload", postgresql.JSONB(astext_type=sa.Text()), nullable=False),
        sa.Column("output_reference_payload", postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    ...
    op.create_table(
        "outbox_events",
        sa.Column("outbox_event_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
    ...
    for table_name in ["ledger_events", "audit_events", "hmrc_api_submissions", "ai_inference_logs"]:
        op.execute(
            f"""
...
def downgrade() -> None:
    ...
    op.execute("DROP TRIGGER IF EXISTS trg_payment_approval_four_eyes ON payment_approvals;")
