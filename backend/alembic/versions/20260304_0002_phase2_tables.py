...
def upgrade() -> None:
    ...
    account_subtype_enum = sa.Enum(
        ...
        "revenue", "cost_of_sales", "operating_expense",
        "finance_income", "finance_expense",
        "fx_revaluation_reserve", "hedging_reserve_oci",
        "intercompany_loan", "cash_pool",
        "cir_adjustment", "interest_payable",
        "interest_receivable", "tax_payable",
    ...
    journal_type_enum = sa.Enum(
        "manual", "auto_vat", "auto_reversal",
        "auto_revaluation", "auto_oci_reclassification",
        "recurring", "year_end_rollover",
        name="journal_type",
    )
    ...
    effectiveness_method_enum = sa.Enum(
        "dollar_offset", "regression",
        name="effectiveness_method",
    )
    for e in [
        vat_treatment_enum, account_subtype_enum, period_status_enum,
        period_type_enum, journal_status_enum, journal_type_enum,
        hedge_type_enum, effectiveness_method_enum,
    ]:
        e.create(bind, checkfirst=True)

    # ---- extend chart_of_accounts ----
    op.add_column("chart_of_accounts", sa.Column(
        "hmrc_nominal_code", sa.String(10), nullable=True,
    ))
    op.add_column("chart_of_accounts", sa.Column(
        "vat_treatment", sa.Enum(
            "T0", "T1", "T2", "T4", "T7", "T9",
            name="vat_treatment_code", create_constraint=False,
        ), nullable=True,
    ))
    op.add_column("chart_of_accounts", sa.Column(
        "account_subtype",
    ...
    op.add_column("chart_of_accounts", sa.Column(
        "is_treasury_account", sa.Boolean(), nullable=False,
        server_default=sa.text("false"),
    ))
    op.add_column("chart_of_accounts", sa.Column(
        "allows_currency_revaluation", sa.Boolean(), nullable=False,
        server_default=sa.text("false"),
    ))
    op.add_column("chart_of_accounts", sa.Column(
        "parent_account_id", postgresql.UUID(as_uuid=True), nullable=True,
    ))
    op.create_foreign_key(
        "fk_coa_parent", "chart_of_accounts",
        "chart_of_accounts", ["parent_account_id"], ["account_id"],
    )

    # ---- accounting_periods ----
    op.create_table(
        "accounting_periods",
        sa.Column("period_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        ...
        sa.Column("period_type", sa.Enum(name="period_type", create_constraint=False), nullable=False),
        sa.Column("period_start", sa.Date(), nullable=False),
        sa.Column("period_end", sa.Date(), nullable=False),
        sa.Column("status", sa.Enum(name="period_status", create_constraint=False),
                  nullable=False, server_default="open"),
        sa.Column("ct_period_utr", sa.String(10), nullable=True),
        sa.Column("ct600_due_date", sa.Date(), nullable=True),
        sa.Column("soft_closed_by_user_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True),
        sa.Column("hard_closed_by_user_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True),
        ...
        sa.UniqueConstraint("tenant_id", "period_start", "period_end", name="uq_period_dates"),
    )
    op.create_index("ix_accounting_periods_tenant_start", "accounting_periods",
    ...
    op.create_table(
        ...
        sa.Column("period_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("accounting_periods.period_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("journal_reference", sa.String(60), nullable=False),
        sa.Column("description", sa.String(255), nullable=True),
        sa.Column("journal_type", sa.Enum(name="journal_type", create_constraint=False),
                  nullable=False, server_default="manual"),
        sa.Column("status", sa.Enum(name="journal_status", create_constraint=False),
                  nullable=False, server_default="draft"),
        sa.Column("currency_code", sa.String(3), nullable=False, server_default="GBP"),
        sa.Column("reversal_of_journal_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("journals.journal_id", ondelete="SET NULL"), nullable=True),
        sa.Column("posted_by_user_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True),
        sa.Column("posted_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("posted_from_ip", sa.String(64), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False,
                  server_default=sa.text("now()")),
        sa.UniqueConstraint("tenant_id", "journal_reference", name="uq_journal_reference"),
    )
    ...
    op.create_table(
        "journal_lines",
        sa.Column("journal_line_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("journal_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("journals.journal_id", ondelete="CASCADE"), nullable=False),
        sa.Column("account_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("chart_of_accounts.account_id", ondelete="RESTRICT"), nullable=False),
        ...
        sa.Column("description", sa.String(255), nullable=True),
        sa.Column("vat_code", sa.Enum(
            "standard", "reduced", "zero", "exempt", "outside_scope",
            name="vat_code", create_constraint=False,
        ), nullable=True),
        sa.Column("vat_treatment", sa.Enum(
            "T0", "T1", "T2", "T4", "T7", "T9",
            name="vat_treatment_code", create_constraint=False,
        ), nullable=True),
    ...
    op.create_index("ix_journal_lines_journal", "journal_lines", ["tenant_id", "journal_id"])
    op.create_index("ix_journal_lines_account", "journal_lines", ["tenant_id", "account_id"])

    # ---- recurring_journal_templates ----
    op.create_table(
        "recurring_journal_templates",
        sa.Column("template_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True),
        ...
        sa.Column("template_lines", postgresql.JSONB(astext_type=sa.Text()), nullable=False),
    ...
    op.create_table(
        "hedge_designations",
        sa.Column("hedge_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("hedge_reference", sa.String(60), nullable=False),
        sa.Column("hedge_type", sa.Enum(name="hedge_type", create_constraint=False), nullable=False),
        ...
        sa.Column("prospective_method", sa.Enum(name="effectiveness_method", create_constraint=False),
                  nullable=False),
        sa.Column("is_active", sa.Boolean(), nullable=False, server_default=sa.text("true")),
        sa.Column("de_designation_date", sa.Date(), nullable=True),
        sa.Column("de_designation_reason", sa.String(500), nullable=True),
        sa.Column("cumulative_oci_treatment_on_dedesignation", sa.String(255), nullable=True),
        sa.Column("tax_note", sa.String(500), nullable=False,
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False,
                  server_default=sa.text("now()")),
        sa.CheckConstraint("hedge_ratio > 0 AND hedge_ratio <= 1", name="ck_hedge_ratio"),
    ...
    op.create_table(
        "hedge_effectiveness_tests",
        sa.Column("test_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("hedge_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("hedge_designations.hedge_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("period_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("accounting_periods.period_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("test_type", sa.String(20), nullable=False),
        sa.Column("method", sa.Enum(name="effectiveness_method", create_constraint=False), nullable=False),
        ...
        sa.Column("tested_by_user_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True),
        sa.Column("tested_at", sa.DateTime(timezone=True), nullable=False,
                  server_default=sa.text("now()")),
        sa.UniqueConstraint("tenant_id", "hedge_id", "period_id", "test_type", name="uq_hedge_test"),
    )
    ...
    op.create_table(
        "hedge_oci_reclassifications",
        sa.Column("oci_reclassification_id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("tenants.tenant_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("hedge_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("hedge_designations.hedge_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("journal_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("journals.journal_id", ondelete="RESTRICT"), nullable=False),
        sa.Column("period_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("accounting_periods.period_id", ondelete="RESTRICT"), nullable=False),
...
def downgrade() -> None:
    op.execute("DROP TRIGGER IF EXISTS trg_hedge_effectiveness_tests_immutable ON hedge_effectiveness_tests;")
    ...
    for col in ["hmrc_nominal_code", "vat_treatment", "account_subtype",
                "is_treasury_account", "allows_currency_revaluation", "parent_account_id"]:
        op.drop_column("chart_of_accounts", col)

    bind = op.get_bind()
    for name in [
        "effectiveness_method", "hedge_type", "journal_type",
        "journal_status", "period_type", "period_status",
        "account_subtype", "vat_treatment_code",
    ]:
