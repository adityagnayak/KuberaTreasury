...
def test_duplicate_camt053_rejected(session_factory):
    ...
    result = StatementIngestionService(session_factory()).ingest_camt053(
        xml_bytes, "user_a"
    )
...
def test_transaction_delete_blocked(session_factory):
    ...
    with session_factory() as session:
        account = (
            session.query(BankAccount).filter_by(iban="DE89370400440532013004").first()
        )
        txn = Transaction(
            trn="TRN-DEL-001",
    ...
    with session_factory() as session:
        ...
        ), "Transaction must still exist after a blocked delete"
...
def test_audit_log_immutable(session_factory):
    ...
    with session_factory() as session:
        ...
        with pytest.raises(Exception) as exc_info:
            ...
            session.commit()
        assert "IMMUTABLE" in str(exc_info.value).upper() or "immutable" in str(
            exc_info.value
        )
        session.rollback()
