...
from app.core.day_count import (
    DayCountConvention, calculate_interest, calculate_year_fraction,
    resolve_convention,
)
from app.core.exceptions import (
    MarketVolatilityAlert, TransferPricingViolationError, UnbalancedJournalError,
)
from app.services.debt_ledger import DebtInstrument, DebtInvestmentLedger
from app.services.fx_risk import (
    FlashCrashDetector, ForwardSettlementAdjuster,
    MockFXRateService, ThreadSafeRateStore, calculate_var,
)
from app.services.gl_engine import GLMappingEngine, TreasuryEvent as GLEvent
...
def test_forward_settlement_business_day_adjustment():
    ...
    adjusted = adjuster.adjust(
        maturity_date=christmas, currency_pair="EUR/GBP", convention="modified_following",
    )
...
def test_interest_accrual_calculation():
    ...
    convention = resolve_convention(instrument.currency, instrument.instrument_subtype or "")
...
def test_gl_journal_must_balance():
    ...
    event = GLEvent(
        event_id="EVT-001", event_type="INTEREST_ACCRUAL",
        amount=Decimal("5000.00"), currency="EUR", metadata={},
    )
...
def test_unbalanced_journal_rejected():
    ...
    engine = GLMappingEngine()
    lines = [
        JournalLine(account_code="1000", account_name="Bank",
                    debit=Decimal("1000"), credit=Decimal("0"), currency="EUR"),
        JournalLine(account_code="4000", account_name="Income",
                    debit=Decimal("0"), credit=Decimal("999"), currency="EUR"),
    ]
...
def test_negative_interest_gl_balances():
    engine = GLMappingEngine()
    event = GLEvent(
        event_id="EVT-002", event_type="INTEREST_ACCRUAL",
        amount=Decimal("-500.00"), currency="EUR", metadata={"negative_rate": True},
    )
    entry = engine.post_journal(event)
    assert sum(l.debit for l in entry.lines) == sum(l.credit for l in entry.lines)
...
def test_fx_revaluation_journal_posted():
    engine = GLMappingEngine()
    event = GLEvent(
        event_id="EVT-003", event_type="FX_REVALUATION",
        amount=Decimal("2500.00"), currency="USD",
        metadata={"gain_loss": Decimal("2500.00"), "direction": "GAIN"},
    )
    entry = engine.post_journal(event)
    names = [l.account_name for l in entry.lines]
...
def test_payment_sent_journal():
    engine = GLMappingEngine()
    event = GLEvent(
        event_id="EVT-004", event_type="PAYMENT_SENT",
        amount=Decimal("10000.00"), currency="EUR", metadata={},
    )
    entry = engine.post_journal(event)
    assert sum(l.debit for l in entry.lines) == sum(l.credit for l in entry.lines)