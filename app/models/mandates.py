...
class Mandate(Base):
    ...
    signatory_user_id: Mapped[Optional[str]] = mapped_column(
    ...
    role: Mapped[Optional[str]] = mapped_column(
        String
    )  # e.g., "A-Signatory", "B-Signatory"
    limit_currency: Mapped[Optional[str]] = mapped_column(String(3))
    single_payment_limit: Mapped[Optional[str]] = mapped_column(
        String
    ...
    documents: Mapped[list["KYCDocument"]] = relationship(
        "KYCDocument", back_populates="mandate"
    )
...
class KYCDocument(Base):
    ...
    mandate_id: Mapped[Optional[str]] = mapped_column(
        String, ForeignKey("mandates.id"), nullable=True
    )
    ...
    file_reference: Mapped[Optional[str]] = mapped_column(
        String, nullable=True
    )  # S3 key or internal path

    expiry_date: Mapped[Optional[date]] = mapped_column(Date, nullable=True)
    uploaded_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)

    mandate: Mapped[Optional["Mandate"]] = relationship(
        "Mandate", back_populates="documents"
    )
